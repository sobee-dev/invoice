...
export default function PreviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // ── Force desktop viewport on this page, restore on leave ──────────
  useEffect(() => {
  ...
  if (loading || !business || !receipt || saving) {
    return <Loader />;
  }
        ...
        <div className="max-w-2xl mx-auto flex flex-col gap-3">
                ...
                {!preparedFiles ? (
                  <>
                    <button
                      ...
                      className="flex-2 h-12 rounded-lg text-black font-bold hover:bg-blue-700 transition-all flex items-center justify-center gap-2"
                    >
                    ...
                    <button
                      onClick={handlePrint}
                      className="flex-1 h-12 rounded-lg border border-slate-300 dark:border-slate-600 text-slate-900 dark:text-white font-semibold"
                    >
                      Print
                    </button>
                  </>
                  ...
                  <div className="flex flex-col w-full gap-3">
                    <div className="flex gap-3">
                      <button
                        onClick={() => handleExecuteShare(preparedFiles.image)}
                        className="flex-1 h-12 rounded-lg bg-green-600 text-white font-bold shadow-lg animate-pulse flex items-center justify-center gap-2"
                      >
                        🚀 Send As Image
                      </button>
                      <button
                        onClick={() => handleExecuteShare(preparedFiles.pdf)}
                        className="flex-1 h-12 rounded-lg bg-slate-900 text-white font-bold shadow-lg animate-pulse flex items-center justify-center gap-2"
                      >
                        🚀 Send As PDF
                      </button>
                    </div>
                    <button
                      onClick={() => setPreparedFiles(null)}
          ...
          {!isDraft && (
            <button
              ...
              className="w-full h-10 text-sm text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200"
            >
              Back to Dashboard
            </button>
          )}
